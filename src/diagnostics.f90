...
REAL(KIND=GP):: momx,momy,momz,tempmom1,tempmom2
...
CALL mpi_reduce(momx,tempmom1,1,mpi_double_precision, mpi_sum,0, &
                mpi_comm_world, ierr)
IF (myrank .EQ. 0) momx = tempmom1
CALL mpi_reduce(momy,tempmom2,1,mpi_double_precision, mpi_sum,0, &
                mpi_comm_world, ierr)
IF (myrank .EQ. 0) momy = tempmom2
...
echem = echem + omega_chem*ABS(psi(i1,i2_loc))**2

eintr = eintr + half*gstrength*ABS(cgp_phys_nlin(i1,i2_loc))*ABS(psi(i1,i2_loc))**2

#ifdef GPPART
!eppot = eppot + REAL(CONJG(psi(i1,i2_loc))*phys_cmpxtmp(i1,i2_loc))
eppot = eppot + REAL(cgp_phys_trans_vobj(i1,i2_loc))*REAL(cgp_phys_nlin(i1,i2_loc))
#endif
...
CALL MPI_REDUCE(eintr,etemp4,1,MPI_DOUBLE_PRECISION,MPI_SUM,&
        0,MPI_COMM_WORLD,ierr)
IF (myrank .EQ. 0) eintr = two*alpha*etemp4*dvol/(lengthx*lengthy)
...
DO idmn = 1, ndim
eobj_kin = eobj_kin + gp_vel_obj(idmn,i1)**2
mom_obj(idmn) = mom_obj(idmn) + gp_mass_part*gp_vel_obj(idmn,i1)
ENDDO
...
WRITE(1,10) time, egrad,echem,eintr,etot
10 FORMAT(E15.6,E15.6,E15.6,E15.6,E15.6)
CLOSE(1)        
...
!rholoc = ABS(cgp_phys_nlin(i1,i2_loc)) +smlnum
rholoc = ABS(psi(i1,i2_loc))**1+smlnum

cgp_phys_tmp1(i1,i2_loc) = CONJG(psi(i1,i2_loc))*cgp_phys_tmp1(i1,i2_loc)/rholoc
cgp_phys_tmp2(i1,i2_loc) = CONJG(psi(i1,i2_loc))*cgp_phys_tmp2(i1,i2_loc)/rholoc
...
!hh = ABS(phys_cmpxtmp(i1,i2_loc))
hh = one!ABS(psi(i1,i2_loc))

!phys_cmpxtmp(i1,i2_loc)  = ABS(psi(i1,i2_loc))
phys_cmpxtmp(i1,i2_loc)  = hh*REAL(cgp_phys_tmp1(i1,i2_loc))
cgp_phys_tmp3(i1,i2_loc) = hh*REAL(cgp_phys_tmp2(i1,i2_loc))

cgp_phys_tmp1(i1,i2_loc) = two*alpha*AIMAG(cgp_phys_tmp1(i1,i2_loc))*hh
cgp_phys_tmp2(i1,i2_loc) = two*alpha*AIMAG(cgp_phys_tmp2(i1,i2_loc))*hh
...
CALL fft(plan_cmpxtmp%pname,forward)
CALL fft(plan_gptmp3%pname,forward)
...
ee1 = four_cmpxtmp(i2,i1_loc)
ee2 = cgp_four_tmp3(i2,i1_loc)
...
!equnt = equnt + equnt_mode*two*alpha**2
...
!IF (MOD(jtime,ssteps)==0) THEN

!IF (myrank .EQ. 0) THEN
!WRITE(fnrst,'(i8)')nrst
!WRITE(fnum,'(i8)') spoint

!OPEN(UNIT=1,FILE='spectra/qspec'//trim(adjustl(fnum))//'.dat',&
!        STATUS='UNKNOWN',IOSTAT=ios)
!DO i1=1,nshell
!WRITE(1,50) dfloat(i1),spec_gp(i1,4)
...
DO idmn = 1, ndim
gp_frc_obj(idmn,i1) = gpfrcpart(idmn)
ENDDO
ENDDO
...
WRITE(1,45) time, gp_pos_obj(1,i1),gp_pos_obj(2,i1)
...
WRITE(1,45) time, gp_vel_obj(1,i1),gp_vel_obj(2,i1)
...
WRITE(1,45) time, gp_frc_obj(1,i1),gp_frc_obj(2,i1)
...
IF (myrank .EQ. 0) THEN
!!--kx = 0 --- averaged in the x-direction
OPEN(UNIT=1,FILE='data/sts1d/kpsi_x0_nrst'//TRIM(ADJUSTL(fnrst))//'.dat',&
FORM='UNFORMATTED', status='UNKNOWN', POSITION='APPEND', IOSTAT=ios)
WRITE(1) (kpsi(i2,1),i2=1,Ny)
CLOSE(1)
ENDIF

CALL mpi_barrier(MPI_COMM_WORLD,ierr)

!!--(ky,kx) --- Slabbed along x-direction 
OPEN(UNIT=1,FILE='data/sts2d/kpsi_nrst'//TRIM(ADJUSTL(fnrst))//'p'//TRIM(ADJUSTL(prc))//'.dat',&
FORM='UNFORMATTED', status='UNKNOWN', POSITION='APPEND', IOSTAT=ios)
WRITE(1) ((kpsi(i2,i1_loc),i2=1,Ny),i1_loc=1,local_Nx_cmpx)
CLOSE(1)
